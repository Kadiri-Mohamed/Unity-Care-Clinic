...
<body>
    ...
    <div class="sidebar d-flex flex-column">
        ...
        <nav class="nav flex-column mt-3">
            ...
            <a href="../routes/router.php?route=admin&doctors" class="nav-link active">
    ...
    <div class="main-content">
            ...
            <div class="container-fluid">
                <div class="navbar-brand">
                    <h5 class="mb-0 text-primary">
                        <i class="fas fa-user-md me-2"></i>Gestion des Médecins
                    </h5>
                </div>
                <div class="d-flex align-items-center">
                    ...
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addDoctorModal">
                        <i class="fas fa-plus me-2"></i>Ajouter Médecin
                    </button>
                </div>
            </div>
        </nav>
        ...
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-list me-2"></i>Liste des Médecins</h5>
            </div>
            <div class="card-body">
                            ...
                            <?php if (!empty($doctors)): ?>
                                <?php foreach ($doctors as $doctor): ?>
                                    <tr>
                                        <td>#<?= $doctor['id'] ?></td>
                                        <td>
                                            <img src="https://ui-avatars.com/api/?name=<?= urlencode($doctor['nom'] . '+' . $doctor['prenom']) ?>&background=134686&color=fff&size=60"
                                                alt="Photo" class="doctor-img">
                                        </td>
                                        <td>
                                            <strong><?= htmlspecialchars($doctor['nom'] . ' ' . $doctor['prenom']) ?></strong><br>
                                            <small
                                                class="text-muted">@<?= htmlspecialchars($doctor['username'] ?? '') ?></small>
                                        </td>
                                        <td>
                                            <span
                                                class="badge bg-info"><?= htmlspecialchars($doctor['specialite'] ?? 'Non spécifié') ?></span>
                                        </td>
                                        <td><?= htmlspecialchars($doctor['email'] ?? '') ?></td>
                                        <td><?= htmlspecialchars($doctor['telephone'] ?? '') ?></td>
                                        <td>
                                            ...
                                            <a href="?profile=<?= $doctor['id'] ?>" class="btn btn-sm btn-info">
    ...
    <div class="modal fade" id="addDoctorModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="../../routes/admin.php?action=create-doctor" method="POST">
    ...
    <script>
        function editDoctor(id) {
            alert('Modifier le médecin #' + id);
        }

        function deleteDoctor(id) {
            if (confirm('Êtes-vous sûr de vouloir supprimer ce médecin ?')) {
                window.location.href = `../../routes/admin.php?action=delete-doctor&id=${id}`;
            }
        }
    </script>
</body>
